...
function SetupDrawer({ setup, connected, close }: { setup: AppSetup; connected: boolean; close: () => void }) {
  const [client, setClient] = useState<"codex" | "claude">("codex"); const [copied, setCopied] = useState(false); const [secret, setSecret] = useState<string>(); const [generating, setGenerating] = useState(false); const [error, setError] = useState("");
  const command = setup.hosted ? secret ? `export CARTOGRAPH_TOKEN=${shellQuote(secret)}\ncodex mcp add cartograph --url ${setup.mcpUrl} --bearer-token-env-var CARTOGRAPH_TOKEN` : "Generate a connection token to reveal the command." : `codex mcp add cartograph -- npm --prefix ${shellQuote(setup.projectRoot)} run start:mcp`;
  ...
  return <><button className="drawer-backdrop" aria-label="Close agent setup" onClick={close}/><aside className="setup-drawer" aria-label="Agent setup"><header><div><small>Agent connection</small><h2>Connect Cartograph</h2></div><button aria-label="Close setup" onClick={close}><X size={16}/></button></header><div className={`connection-state ${connected ? "connected" : ""}`}><i/><span><b>{connected ? "Connected" : "Waiting for an agent"}</b><small>{connected ? "Cartograph received an authenticated MCP request." : "Complete setup, then return here to confirm the connection."}</small></span></div><nav aria-label="Agent client"><button className={client === "codex" ? "active" : ""} onClick={() => { setClient("codex"); setCopied(false); }}>Codex</button><button className={client === "claude" ? "active" : ""} onClick={() => { setClient("claude"); setCopied(false); }}>Claude Code</button></nav><section><span>01</span><div><h3>{setup.hosted && !secret ? "Create a secure connection" : client === "codex" ? "Run this command" : "Add this MCP server"}</h3><p>{setup.hosted && !secret ? "Generate a revocable token. It is shown once and stored only as a secure hash." : client === "codex" ? "Run it once in Terminal. Codex stores the remote server in its MCP configuration." : "Paste this JSON into your Claude Code MCP configuration, then restart the client."}</p>{setup.hosted && !secret ? <button className="generate-token" disabled={generating} onClick={() => void generate()}>{generating ? <LoaderCircle className="spin" size={14}/> : <Terminal size={14}/>}Generate connection token</button> : <pre><code>{value}</code><button aria-label="Copy setup" onClick={() => void copy()}>{copied ? <Check size={14}/> : <Copy size={14}/>} {copied ? "Copied" : "Copy"}</button></pre>}{error && <small className="form-error">{error}</small>}</div></section><section><span>02</span><div><h3>Start a fresh agent session</h3><p>Open the repository in your agent and ask it to use Cartograph. The connection status updates after its first tool call.</p></div></section><section><span>03</span><div><h3>Create the first map</h3><p>Close this panel and copy one of the starter investigations from the workspace.</p></div></section></aside></>;
}
