...
export function App() {
  ...
  if (!repositories.length) return <Onboarding onComplete={refreshRepositories} />;
  return <main className="shell">
    {mobileNav && <button className="nav-backdrop" aria-label="Close navigation" onClick={() => setMobileNav(false)} />}
    <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
      <div className="brand"><div className="brand-mark"><Map size={17} /></div><b>Cartograph</b><button className="mobile-close" aria-label="Close navigation" onClick={() => setMobileNav(false)}><X size={16}/></button></div>
      <div className="repo-control">
        <button className="repo-switcher" aria-expanded={repoMenu} onClick={() => setRepoMenu(value => !value)}><FolderGit2 size={15} /><span><small>Repository</small><b>{repo?.name}</b><em>{repo?.rootPath}</em></span><ChevronDown size={14} /></button>
        {repoMenu && <div className="repo-menu">{repositories.map(item => <div className="repo-menu-item" key={item.id}><button className={item.id === repo?.id ? "active" : ""} onClick={() => { setRepo(item); setRepoMenu(false); setSelected(undefined); }}><FolderGit2 size={13}/><span>{item.name}<small>{item.rootPath}</small></span></button><button className="repo-delete" aria-label={`Remove ${item.name}`} title={`Remove ${item.name}`} onClick={() => { setRepositoryToDelete(item); setRepoMenu(false); }}><Trash2 size={13}/></button></div>)}</div>}
      </div>
...
function DataLens({ payload }: { payload: NonNullable<KnowledgeTrace["steps"][number]["payload"]> }) {
  const [selected, setSelected] = useState(0); const field = payload.fields[selected];
  return <div className="data-lens"><header><span><Braces size={12}/>Data lens</span><i>{payload.format}</i></header><div className="payload-diff"><div><small>Before</small><pre>{formatPayload(payload.before)}</pre></div><ArrowRight size={14}/><div><small>After</small><pre>{formatPayload(payload.after)}</pre></div></div>{payload.fields.length > 0 && <div className="field-changes"><nav aria-label="Payload field changes">{payload.fields.map((item, itemIndex) => <button className={`${item.operation} ${itemIndex === selected ? "active" : ""}`} key={`${item.path}-${itemIndex}`} onClick={() => setSelected(itemIndex)}><i>{operationMark(item.operation)}</i><span>{item.path}</span><small>{item.operation.replace("_", " ")}</small></button>)}</nav>{field && <aside><b>{field.path}</b><p>{field.explanation}</p>{(field.before !== undefined || field.after !== undefined) && <code>{formatInline(field.before)} <ArrowRight size={10}/> {formatInline(field.after)}</code>}{field.evidence[0] && <em>{field.evidence[0].path}{field.evidence[0].startLine ? `:${field.evidence[0].startLine}` : ""}</em>}</aside>}</div>}</div>;
}
...
function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setBusy(true); setError(""); try { await api.register(path); await onComplete(); } catch (err) { setError(message(err)); } finally { setBusy(false); } }
  ...
  return <main className="onboarding"><div className="onboard-brand"><Map size={18}/><b>Cartograph</b></div><section><div className="kicker"><Sparkles size={14}/>Persistent code understanding</div><h1>Turn agent investigations into maps you can keep.</h1><p>Add a repository workspace, then let Codex, Claude Code, or any MCP client save visual explanations with source references.</p><form onSubmit={submit}><label>Absolute repository path</label><div className={dragging ? "dragging" : ""} onDragEnter={event => { event.preventDefault(); setDragging(true); }} onDragOver={event => event.preventDefault()} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as globalThis.Node | null)) setDragging(false); }} onDrop={drop}><Code2 size={17}/><input value={path} onChange={e => setPath(e.target.value)} placeholder="/Users/you/code/repository" required/><button disabled={busy}>{busy ? <LoaderCircle className="spin" size={17}/> : "Add repository"}</button></div><small className="path-hint">Cartograph stores the path without scanning or parsing your code.</small>{error && <small className="form-error">{error}</small>}</form></section><footer>Local-first · Source stays on your machine · SQLite-backed</footer></main>;
}
...
const absoluteSource = (repository: Repository, evidence: Pick<Evidence, "path" | "startLine">) => `${repository.rootPath.replace(/\/$/, "")}/${evidence.path}${evidence.startLine ? `:${evidence.startLine}` : ""}`;
