  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <header className="topbar">
        ...
        <div className="title-block">{view?.ancestors.length ? <div className="map-path"><button aria-label="Go back one level" onClick={() => void openViewById(view.ancestors.at(-1)!.viewId)}><ArrowLeft size={12}/></button>{view.ancestors.map(ancestor => <span key={ancestor.viewId}><button onClick={() => void openViewById(ancestor.viewId)}>{ancestor.title}</button><ChevronRight size={10}/></span>)}<b>{view.title}</b></div> : <div className="eyebrow"><span>Knowledge map</span><ArrowRight size={11} /><span>{repo?.name}</span></div>}<div className="map-heading"><h1>{view?.ancestors.length ? view.ancestors.at(-1)?.nodeLabel : view?.title ?? "No view selected"}</h1>{view && <span className="map-level"><b>{view.viewType}</b> view{view.scope && <><i />{view.scope}</>}</span>}</div></div>
