...
export function createCartographMcp(store: any, options: { hosted: boolean; activity?: () => void }) {
  ...
  server.tool("create_trace", "Save a step-by-step journey through an existing view. Steps must reference nodes in that view. Payload field modifiers must use real JSON paths and agree with before/after: added is absent→present, removed is present→absent, modified changes an existing value, read exists before, persisted exists after, and redacted points to [REDACTED]. Represent HTTP or other external output with produces, not a synthetic payload field. Use repository-relative source references and synthetic payloads without secrets or personal data.", { viewId: z.string(), title: z.string(), description: z.string().default(""), steps: z.array(TraceStepSchema).min(2).max(24) }, async input => { active(); try { return result(await store.createTrace(CreateTraceSchema.parse(input))); } catch (error) { return { isError: true, ...result({ error: error instanceof Error ? error.message : "Could not create trace" }) }; } });
