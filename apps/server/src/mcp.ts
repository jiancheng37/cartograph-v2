...
server.tool("list_repositories", "List repositories registered with Cartograph. Call this before creating a view. If the current repository is absent, call register_repository.", {}, async () => { touchMcp(database); return result(store.repositories()); });
...
server.tool("list_traces", "List saved step-by-step journeys for a view.", { viewId: z.string() }, async ({ viewId }) => { touchMcp(database); return result(store.traces(viewId)); });
server.tool("find_views", "Find existing visual knowledge. Use this before answering so prior understanding can be reused.", { repositoryId: z.string().optional(), query: z.string().default("") }, async ({ repositoryId, query }) => { touchMcp(database); return result(store.views(repositoryId).filter(view => `${view.title} ${view.description}`.toLowerCase().includes(query.toLowerCase()))); });
server.tool("get_view", "Retrieve a complete saved view including nodes, relationships, source references, and confidence.", { viewId: z.string() }, async ({ viewId }) => { touchMcp(database); const view = store.view(viewId); return view ? result(view) : { isError: true, ...result({ error: "View not found" }) }; });
server.tool("create_view", "Create a compact visual explanation. To explain a node in more detail without changing its diagram, pass parentViewId and parentNodeId to create a drill-down view. Prefer 5-12 semantic nodes, stable IDs, concise summaries, and source references.", { title: z.string(), description: z.string().default(""), repositoryId: z.string(), parentViewId: z.string().optional(), parentNodeId: z.string().optional(), nodes: z.array(nodeSchema).max(80), edges: z.array(edgeSchema).max(160) }, async input => { touchMcp(database); return result(store.createView(CreateViewSchema.parse(input))); });
