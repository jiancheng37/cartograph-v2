...
export function registerRepository(store: Store, pathInput: string): Repository {
  ...
  const existing = store.repositories().find(repository => repository.rootPath === rootPath);
